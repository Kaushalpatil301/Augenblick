...
const SOCKET_URL = import.meta.env.VITE_SERVER_URL?.replace("/api/v1/", "") || "http://localhost:8000";
        ...
        <div className="flex flex-col h-[600px] w-full rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden min-h-[500px]">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {messages.map((msg, idx) => {
                    ...
                    const senderInfo = typeof msg.sender === 'object' ? msg.sender : members?.find(m => m._id === msg.sender);
                        ...
                        >
                            {!isMe && (
                                <Avatar className="h-8 w-8">
                                    ...
                                    <AvatarFallback>{senderInfo?.username?.[0]?.toUpperCase() || <Bot size={14} />}</AvatarFallback>
                                </Avatar>
                            )}
                            <div className={`max-w-[75%] px-4 py-2 rounded-2xl text-sm shadow-sm ${isMe ? "bg-indigo-600 text-white rounded-br-sm" : "bg-gray-100 text-gray-800 rounded-bl-sm"
                                }`}>
                                {!isMe && <p className="text-[10px] font-medium text-indigo-600 mb-1">{senderInfo?.username || "Agentic AI"}</p>}
                                <p>{msg.text}</p>
                            </div>
                        </motion.div>
                    );
            ...
            <form onSubmit={handleSendMessage} className="p-3 bg-white border-t border-gray-100 flex gap-2">
                ...
                />
                <Button type="submit" size="icon" className="rounded-full bg-indigo-600 hover:bg-indigo-700 shrink-0 text-white">
                    <Send size={16} />
                </Button>
            </form>
        </div>
