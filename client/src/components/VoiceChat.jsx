...
export default function VoiceChat() {
  ...
  const handleDisconnect = () => {
    try {
      ...
      const voiceSocket = io(SOCKET_URL, { withCredentials: true });
      voiceSocket.on("connect", () => {
        voiceSocket.emit("end_voice_chat", {
          tripId,
          sender: user?._id || "guest",
          username: user?.username || "A user",
        });
        setTimeout(() => voiceSocket.close(), 500);
      });
    } catch (err) {
      console.error("Failed to emit end_voice_chat", err);
    }
