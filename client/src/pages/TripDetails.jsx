...
export default function TripDetails() {
  ...
  const fetchTrip = async () => {
    try {
      ...
      const tripData = tripRes.status === 'fulfilled' ? tripRes.value.data.data : null;
      const playgroundData = playgroundRes.status === 'fulfilled' ? playgroundRes.value.data.data : null;
      ...
      if (playgroundData && Object.keys(playgroundData).length > 0) {
        ...
        Object.keys(playgroundData).forEach(category => {
          if (playgroundData[category] && playgroundData[category].length > 0) {
            ...
            console.log(`[Playground] Loaded ${playgroundData[category].length} ${category} items from n8n collections`);
          }
        });
      }
  ...
  const handleRouteMapSearchSelect = async (result) => {
    ...
    try {
      ...
      const city = addr.city || addr.town || addr.village || addr.county || addr.state || result.display_name.split(",")[0];
  ...
  const openRouteEditor = () => {
    setRouteForm({
      ...
      mainDestination: trip?.mainDestination?.city ? trip.mainDestination: null,
    });
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <Tabs defaultValue="overview" className="w-full">
        <div className="flex items-center justify-between mb-4">
          <Button variant="ghost" onClick={() => navigate("/dashboard/trips")} className="gap-2">
            <ArrowLeft size={16} /> Back to Trips
          </Button>
          <div className="flex flex-wrap items-center justify-end gap-2">
            <TabsList className="h-10 bg-transparent p-0 gap-2">
              <TabsTrigger value="overview" className="h-10 px-4 text-sm font-medium border rounded-md data-[state=active]:bg-accent">Overview</TabsTrigger>
              <TabsTrigger value="map" className="h-10 px-4 text-sm font-medium border rounded-md data-[state=active]:bg-accent">Map</TabsTrigger>
              <TabsTrigger value="planning" className="h-10 px-4 text-sm font-medium border rounded-md data-[state=active]:bg-accent">Planning</TabsTrigger>

            </TabsList>
            <Button variant="outline" className="gap-2" onClick={() => setItineraryOpen(true)}>
            ...
            {trip.createdBy?._id === currentUserId && (
              <Button variant="outline" className="gap-2 text-red-600 border-red-200" onClick={handleDeleteTrip}>
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
              <div>
                ...
                <div className="flex flex-wrap items-center gap-2 mt-2 opacity-90">
                  <span className="flex items-center gap-1">
                    <MapPin size={16} /> {trip.mainDestination?.city || "Discovery Pending..."}
                  </span>
              ...
              <div className="text-right">
                <p className="text-xs opacity-70 uppercase tracking-widest font-bold">Total Budget</p>
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            {[
              { key: "origin", label: "Origin", place: trip.origin, color: "emerald" },
              { key: "dest", label: "Main Destination", place: trip.mainDestination, color: "rose" }
            ].map((loc) => (
              <Card key={loc.key} className="overflow-hidden border-none shadow-md">
                <div className="relative aspect-[16/9] bg-gray-100">
                  {placeImages[loc.key] ? (
                    <img src={placeImages[loc.key]} className="absolute inset-0 w-full h-full object-cover" />
                  ) : (
                    <div className="absolute inset-0 flex items-center justify-center"><MapPin className="text-gray-300" size={40} /></div>
                  )}
                  <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/60 to-transparent">
                    <p className="text-[10px] font-bold text-white/80 uppercase tracking-widest">{loc.label}</p>
                    <h4 className="text-lg font-bold text-white">{loc.place?.city || "Select in Mapping"}</h4>
                  </div>
                </div>
              </Card>
            ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
            {sectionData.map((sec) => {
              const hasItems = sec.userItems.length > 0 || sec.aiItems.length > 0;
              return (
                <Card key={sec.title} className="bg-white/50 backdrop-blur-sm border-gray-100">
                  <CardHeader className="py-3 px-4">
                    <CardTitle className="text-xs font-bold flex items-center gap-2 text-gray-600">
                      <sec.icon size={14} className={`text-${sec.color}-500`} /> {sec.title}
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="px-4 pb-4">
                    {!hasItems ? (
                      <p className="text-[10px] text-gray-400 italic">{sec.emptyText}</p>
                    ) : (
                      <div className="flex flex-col gap-1.5">
                        {sec.userItems.slice(0, 1).map((item, i) => (
                          <div key={i} className="text-[11px] font-bold text-gray-800 truncate">
                            {item.name || item.restaurantName || item.type}
                          </div>
                        ))}
                        {sec.aiItems.slice(0, 1).map((item, i) => (
                          <div key={i} className="text-[11px] text-indigo-600 font-medium truncate flex items-center gap-1">
                            <Sparkles size={8} /> {item.name || item.hotel_name || item.provider || item.restaurantName}
                          </div>
                        ))}
                        {(sec.userItems.length + sec.aiItems.length > 1) && (
                          <p className="text-[9px] text-gray-400 mt-0.5">+{sec.userItems.length + sec.aiItems.length - 1} more</p>
                        )}
                      </div>
                    )}
                  </CardContent>
        ...
        <TabsContent value="map" className="mt-6">
          <div className="mb-3 flex items-center justify-between rounded-lg border border-indigo-100 bg-indigo-50/50 px-3 py-2">
            ...
            <span className="text-[11px] text-indigo-600">Overview, Map and Planning synced</span>
          </div>
                  ...
                  {sec.aiItems.length === 0 ? (
                    ...
                    <div className="space-y-1.5">
                      {sec.aiItems.slice(0, 2).map((item, i) => (
                        <p key={i} className="text-[11px] text-indigo-700 truncate flex items-center gap-1">
                          <Sparkles size={8} /> {item.name || item.hotel_name || item.provider || item.restaurantName}
                        </p>
                      ))}
                      {sec.aiItems.length > 2 && (
                        <p className="text-[10px] text-gray-500">+{sec.aiItems.length - 2} more</p>
                      )}
                    </div>
                  )}
                </CardContent>
                          ...
                          >
                            <div className="min-w-0">
                              <p className="font-bold text-sm text-indigo-900 truncate">
                                {item.name || item.hotel_name || item.provider || item.restaurantName}
                              </p>
                              <p className="text-[11px] text-indigo-700/70 truncate">
                                {item.neighborhood || item.mode || "Agent Finding"}
                              </p>
                            </div>
        ...
        <DialogContent className="sm:max-w-3xl h-[80vh]">
          ...
          <div className="space-y-4 h-full flex flex-col">
            <div className="flex gap-2">
              <Button size="sm" variant={routePickMode === "origin" ? "default" : "outline"} onClick={() => setRoutePickMode("origin")}>Origin</Button>
              <Button size="sm" variant={routePickMode === "dest" ? "default" : "outline"} onClick={() => setRoutePickMode("dest")}>Destination</Button>
              <div className="flex-1" />
              <Button size="sm" onClick={handleSaveRoute} disabled={savingRoute}>{savingRoute ? <Loader2 className="animate-spin" /> : <Check />} Save</Button>
            </div>
            <div className="flex-1 bg-gray-100 rounded-xl overflow-hidden min-h-[300px]">
               <MapContainer center={[20, 0]} zoom={2} style={{ height: "100%" }} ref={routeMapRef}>
                 <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                 <RouteMapClickHandler onPick={handleRouteMapClick} />
                 {routeForm.origin && <Marker position={[routeForm.origin.lat, routeForm.origin.lng]} icon={routeOriginIcon} />}
                 {routeForm.mainDestination && <Marker position={[routeForm.mainDestination.lat, routeForm.mainDestination.lng]} icon={routeDestIcon} />}
                 {routeForm.origin && routeForm.mainDestination && (
                   <Polyline positions={[[routeForm.origin.lat, routeForm.origin.lng], [routeForm.mainDestination.lat, routeForm.mainDestination.lng]]} pathOptions={{ color: "#3b82f6", dashArray: "8 6" }} />
                 )}
               </MapContainer>
            </div>
          </div>
      ...
      <Dialog open={chatOpen} onOpenChange={setChatOpen}>
        <DialogContent className="max-w-md h-[600px] p-0"><TripChat tripId={trip._id} members={trip.members} /></DialogContent>
      </Dialog>
      
      <Dialog open={!!showAddForm} onOpenChange={() => setShowAddForm(null)}>
        <DialogContent>
          <form onSubmit={handleAddDetail} className="space-y-4 pt-4">
             <Label className="capitalize">Add New {showAddForm}</Label>
             <Input name="name" placeholder="Name/Title" required onChange={handleInputChange} />
             <Input name="location" placeholder="Address/Location" onChange={handleInputChange} />
